...
public class TagDeltaCalc extends TableImpl<TagDeltaCalcRecord> {
    ...
    private TagDeltaCalc(Name alias, Table<TagDeltaCalcRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"tag_delta_calc\" as  SELECT delta.added,\n    delta.source,\n    delta.tag,\n    delta.image_id\n   FROM ( SELECT true AS added,\n            child.tag_family AS source,\n            child.tag_value AS tag,\n            child.image_id\n           FROM image_tags child\n          WHERE (NOT (EXISTS ( SELECT 1\n                   FROM image_tags parent,\n                    parent_child_image pci\n                  WHERE ((pci.parent_image_id = parent.image_id) AND (pci.child_image_id = child.image_id) AND ((child.tag_value)::text = (parent.tag_value)::text) AND ((child.tag_family)::text = (parent.tag_family)::text)))))\n        UNION\n         SELECT false AS added,\n            parent.tag_family AS source,\n            parent.tag_value AS tag,\n            parent.image_id\n           FROM image_tags parent\n          WHERE (NOT (EXISTS ( SELECT 1\n                   FROM image_tags child,\n                    parent_child_image pci\n                  WHERE ((pci.parent_image_id = parent.image_id) AND (pci.child_image_id = child.image_id) AND ((child.tag_value)::text = (parent.tag_value)::text) AND ((child.tag_family)::text = (parent.tag_family)::text)))))) delta;"));
    }
